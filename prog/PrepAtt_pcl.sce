...
screen_height = 768;
screen_bit_depth = 32;
default_background_color = 0,0,0;#200, 200, 200;
...
int dur_sTAR=200;			#target sound duration
int dur_sDIS=300;
int dur_cue=200;			#cue presentation 198 (a verifier dans TEMPLATE "PrepAtt_stim_trial.tem") pour 200 dans les trials = 15 trames a 75Hz
double correc_atttar =-0.15;
...
if !inatt.last_succeeded() then 
 term.print( "Error reading attenuation file !\n" );
 exit();
else end;
...
#====attribution des noms d'événements et trials
   ...
   Sound_tar_wav.set_filename(TAR);
	...
	#cue + dis
   if (cue=="Ldog") then
     event_cue.set_stimulus(Ldog);
     event_cue_dis.set_stimulus(Ldog);
     cuefic=1;
     end;
   if (cue=="Rdog") then
     event_cue.set_stimulus(Rdog);
     event_cue_dis.set_stimulus(Rdog);
     cuefic=2;
   end;
   if (cue=="Bdog") then
     event_cue.set_stimulus(Bdog);
     event_cue_dis.set_stimulus(Bdog);
     cuefic=0;
   end;
   if (DIS!="nul.wav") then
...
#==================affichage moyenne================
text1.set_caption("temps de reaction "+string(floor(moytr))); 
text1.redraw();
info_trial.present();
